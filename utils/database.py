...
def init_user_favorites_table():
    """Initializes the user_favorites table for storing user favorites (restaurants and menu items)."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
    ...
    conn.commit()
    conn.close()
    log("User favorites table initialized.")
